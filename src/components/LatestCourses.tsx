...
const LatestCourses = () => {
  ...
  const handleCourseClick = (course: TutorCourse) => {
    const courseUrl = getCourseLink(course);
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
              {courses.map((course) => {
                ...
                const totalEnrolled = getTotalEnrolled(course);
                      ...
                      <div className="grid grid-cols-2 gap-3 mb-4 text-sm">
                        ...
                        {totalEnrolled > 0 && (
                          <div className="flex items-center gap-2 text-muted-foreground">
                            ...
                            <span>{totalEnrolled} طالب</span>
                          </div>
                        )}
                        {totalAssignments > 0 && (
                          <div className="flex items-center gap-2 text-muted-foreground">
                            <Clipboard className="h-4 w-4" />
                            <span>{totalAssignments} واجب</span>
                          </div>
                        )}
                        {totalQuizzes > 0 && (
                          <div className="flex items-center gap-2 text-muted-foreground">
                            <FileText className="h-4 w-4" />
                            <span>{totalQuizzes} اختبار</span>
                          </div>
                        )}
                      </div>
